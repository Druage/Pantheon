...
# Get Game page
def _get_game_page_url(system,search, new_platform):
    platform = new_platform
    results = []
    try:
        req = urllib.request.Request('http://thegamesdb.net/api/GetGamesList.php?name='+urllib.parse.quote_plus(search)+'&platform='+urllib.parse.quote_plus(platform))
...
def _get_thumbnails_list(system,search,region,imgsize):
    ...
    try:
        req = urllib.request.Request(game_id_url)
        req.add_unredirected_header('User-Agent', 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.31 (KHTML, like Gecko) Chrome/26.0.1410.64 Safari/537.31')
        f = urllib.request.urlopen(req)
        page = f.read().replace('\n', '')
